#ifndef _FUNCTIONALITIES_H_
#define _FUNCTIONALITIES_H_

/* These two macros activate or deactivate main code */
#define SPECTS_BASIC true
#define SPECTS_EXTRA true

/*Basic Features*/
#define CONTROL_EMERGERCY_DOOR false
#define CONTROL_CAPACITY true
#define CONTROL_SMART_CORRIDOR_LIGHT true
#define CONTROL_TEMP false
#define CONTROL_EXTERNAL_LIGHT false
#define CONTROL_HUMIDITY false

/*Extra Features*/
#define IN_OUT_BARRIER false
#define IN_OUT_RFID true
#define IN_OUT_MANUAL true

#endif
